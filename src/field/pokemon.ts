...
export default abstract class Pokemon extends Phaser.GameObjects.Container {
  ...
  constructor(
    ...
    const hiddenAbilityChance = new Utils.IntegerHolder(
      BASE_HIDDEN_ABILITY_CHANCE
    );
    if (!this.hasTrainer()) {
  ...
  async populateVariantColorCache(
    ...
    const spritePath = `./images/pokemon/variant/${
      useExpSprite ? "exp/" : ""
  ...
  getCritStage(source: Pokemon, move: Move): number {
    ...
    if (critBoostTag) {
      if (critBoostTag instanceof DragonCheerTag) {
        critStage.value += critBoostTag.typesOnAdd.includes(Type.DRAGON)
          ? 2
          : 1;
      } else {
        critStage.value += 2;
  ...
  calculateStats(): void {
    ...
    for (const s of PERMANENT_STATS) {
      const statHolder = new Utils.IntegerHolder(
  ...
  public getLearnableLevelMoves(): Moves[] {
    let levelMoves = this.getLevelMoves(1, true, false, true).map(
      (lm) => lm[1]
    );
    if (
      this.metBiome === -1 &&
  ...
  public getTypes(
    ...
    if (!types.length || !includeTeraType) {
      if (
        ...
        const firstType =
          customTypes && this.customPokemonData.types[0] !== Type.UNKNOWN
        ...
        if (fusionSpeciesForm) {
          ...
          if (
            ...
            secondType =
              ...
              this.customPokemonData.types[1] !== Type.UNKNOWN
                ? this.customPokemonData.types[1]
                : speciesForm.type2 ?? Type.UNKNOWN;
          }
        } else {
  ...
  getAttackTypeEffectiveness(
    ...
    const typeMultiplierAgainstFlying = new Utils.NumberHolder(
      getTypeDamageMultiplier(moveType, Type.FLYING)
    );
    applyChallenges(
      globalScene.gameMode,
      ChallengeType.TYPE_EFFECTIVENESS,
  ...
  public generateAndPopulateMoveset(): void {
    ...
    if (this.isBoss()) {
      movePool = movePool.filter(
        (m) => !allMoves[m[0]].hasAttr(SacrificialAttr)
      );
  ...
  getAccuracyMultiplier(target: Pokemon, sourceMove: Move): number {
    ...
    const targetEvaStage = new Utils.IntegerHolder(
  ...
  apply(source: Pokemon, move: Move): HitResult {
    ...
    if (moveCategory.value === MoveCategory.STATUS) {
      ...
      const noCritTag = globalScene.arena.getTagOnSide(
        NoCritTag,
        defendingSide
      );
      ...
      const typeBoost = source.findTag(
        (t) =>
          ...
          t.boostedType === source.getMoveType(move)
      ) as TypeBoostTag;
      if (typeBoost?.oneUse) {
  ...
  isMax(): boolean {
    ...
    return (
      ...
      (!!this.getFusionFormKey() && maxForms.includes(this.getFusionFormKey()!))
    );
  }
...
export class PlayerPokemon extends Pokemon {
  ...
  addFriendship(friendship: number): void {
    if (friendship > 0) {
      ...
      const starterAmount = new Utils.NumberHolder(
        Math.floor((amount.value * candyFriendshipMultiplier) / fusionReduction)
      );
      ...
      globalScene.ui.setMode(
        ...
        (slotIndex: number, option: PartyOption) => {
          if (slotIndex >= 0 && slotIndex < 6) {
            ...
            pokemon.heal(
              Math.min(
                Utils.toDmgValue(0.5 * pokemon.getMaxHp()),
                pokemon.getMaxHp()
              )
            );
            globalScene.queueMessage(
  ...
  evolve(
    ...
    return new Promise((resolve) => {
      ...
      if (preEvolution.speciesId === Species.GIMMIGHOUL) {
        const evotracker =
          this.getHeldItems().filter(
            (m) => m instanceof EvoTrackerModifier
          )[0] ?? null;
        if (evotracker) {
    ...
    if (
      ...
      if (newEvolution.condition?.predicate(this)) {
        ...
        const modifiers = globalScene.findModifiers(
          (m) =>
            m instanceof PokemonHeldItemModifier && m.pokemonId === this.id,
          true
        ) as PokemonHeldItemModifier[];
        modifiers.forEach((m) => {
  ...
  fuse(pokemon: PlayerPokemon): Promise<void> {
    return new Promise((resolve) => {
      ...
      this.generateCompatibleTms();
      this.updateInfo(true);
      const fusedPartyMemberIndex = globalScene
        .getPlayerParty()
        .indexOf(pokemon);
      ...
      const fusedPartyMemberHeldModifiers = globalScene.findModifiers(
        (m) =>
          m instanceof PokemonHeldItemModifier && m.pokemonId === pokemon.id,
        true
      ) as PokemonHeldItemModifier[];
      ...
      Promise.allSettled(transferModifiers).then(() => {
        globalScene.updateModifiers(true, true).then(() => {
          ...
          const newPartyMemberIndex = globalScene
            .getPlayerParty()
            .indexOf(this);
          pokemon
            .getMoveset(true)
