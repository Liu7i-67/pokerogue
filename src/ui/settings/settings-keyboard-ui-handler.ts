...
export default class SettingsKeyboardUiHandler extends AbstractControlSettingsUiHandler {
  ...
  constructor(scene: BattleScene, mode?: Mode) {
    ...
    const deleteEvent = scene.input.keyboard.addKey(
      Phaser.Input.Keyboard.KeyCodes.DELETE
    );
    const restoreDefaultEvent = scene.input.keyboard.addKey(
  ...
  setup() {
    ...
    iconDelete.setPositionRelative(
      this.actionsBg,
      this.navigationContainer.width - 260,
      4
    );
  ...
  onHomeDown(): void {
    if (
      ![Mode.SETTINGS_KEYBOARD, Mode.SETTINGS_GAMEPAD].includes(
        this.scene.ui.getMode()
      )
    ) {
      return;
    }
