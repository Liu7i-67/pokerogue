...
export default class BattleScene extends SceneBase {
  ...
  loadPokemonAtlas(key: string, atlasPath: string, experimental?: boolean) {
    ...
    this.load.atlas(
      key,
      `images/pokemon/${variant ? "variant/" : ""}${
        experimental ? "exp/" : ""
      }${atlasPath}.png`,
      `images/pokemon/${variant ? "variant/" : ""}${
        experimental ? "exp/" : ""
  ...
  getDoubleBattleChance(newWaveIndex: number, playerField: PlayerPokemon[]) {
    const doubleChance = new Utils.IntegerHolder(
    ...
    return new Promise((resolve) => {
      ...
      if (modifier instanceof PersistentModifier) {
          ...
          modifiersToRemove.push(
            ...this.findModifiers(
              (m) =>
                ...
                m.pokemonId === modifier.pokemonId
            )
          );
        }
        ...
        if (!ignoreUpdate && !virtual) {
          return this.updateModifiers(true, instant).then(() =>
            resolve(success)
          );
        }
    ...
    return new Promise((resolve) => {
      ...
      Utils.executeIf(!!source && source.isPlayer() !== target.isPlayer(), () =>
        ...
        if (
          ...
          if (
            source &&
            source.isPlayer() !== target.isPlayer() &&
            !ignoreUpdate
          ) {
            this.updateModifiers(source.isPlayer(), instant).then(() =>
              addModifier()
            );
          } else {
            addModifier();
      ...
      party.forEach((enemyPokemon: EnemyPokemon, i: number) => {
        if (
          ...
          if (
            ...
            pokemonModifierChance = Math.ceil(
              pokemonModifierChance *
                this.currentBattle.trainer.getPartyMemberModifierChanceMultiplier(
                  i
                )
            ); // eslint-disable-line
          let count = 0;
      ...
      party: this.party
        ? this.party.map((p) => {
            return {
              ...
              teraType:
                p.getTeraType() !== Type.UNKNOWN ? Type[p.getTeraType()] : "",
              level: p.level,
              currentHP: p.hp,
              maxHP: p.getMaxHp(),
